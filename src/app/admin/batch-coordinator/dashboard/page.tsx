...
export default function BatchCoordinatorDashboard() {
  ...
  useEffect(() => {
    ...
    if (foundDept) {
      ...
      const totalSubjects = batchSubjects.length;
    ...
    <div className="p-4 md:p-6 lg:p-12 font-sans min-h-screen">
      <div className="mb-10 flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        <div>
          <h1 className="text-4xl font-black uppercase italic tracking-tighter text-slate-900 flex items-center gap-3">
             <Target size={36} className="text-teal-600" />
             <span className="text-teal-600">{course} • {batch}</span>
          </h1>
          <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mt-2">
            Batch Co-ordinator Dashboard • {department || "Department"}
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
        <motion.div whileHover={{ y: -4 }} className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-xl relative overflow-hidden group">
          <div className="absolute top-0 right-0 w-32 h-32 bg-teal-50 rounded-full blur-3xl -mr-10 -mt-10 transition-transform group-hover:scale-150" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-[2rem] border border-slate-100 shadow-xl p-8">
           <h2 className="text-lg font-black italic uppercase tracking-tighter text-slate-900 mb-6 flex items-center gap-2">
             <Clock size={20} className="text-teal-500" /> Batch Deadlines
           </h2>
           <div className="space-y-4">
             <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100">
               <div className="flex justify-between items-start mb-2">
                 <h3 className="text-sm font-bold text-slate-900">Finalize Internal Marks</h3>
               ...
               <p className="text-xs font-bold text-slate-500">Submit internal marks for {batch} to the HOD.</p>
             </div>
             <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100">
               <div className="flex justify-between items-start mb-2">
                 <h3 className="text-sm font-bold text-slate-900">Parent-Teacher Meet</h3>
                 <span className="text-[9px] font-black uppercase tracking-widest text-blue-600 bg-blue-100 px-2 py-1 rounded-md">Next Week</span>
               </div>
               <p className="text-xs font-bold text-slate-500">Prepare performance reports for {batch} students.</p>
             </div>
           </div>
        </div>
        ...
        <div className="bg-white rounded-[2rem] border border-slate-100 shadow-xl p-8">
           <h2 className="text-lg font-black italic uppercase tracking-tighter text-slate-900 mb-6 flex items-center gap-2">
             <GraduationCap size={20} className="text-teal-500" /> At-Risk Students
           </h2>
           <div className="flex flex-col items-center justify-center py-10 bg-slate-50 rounded-2xl border-2 border-dashed border-slate-200">
              ...
              <p className="text-xs font-bold text-slate-400 mt-2 text-center px-4">All {batch} students are currently maintaining satisfactory attendance and grades.</p>
           </div>
        </div>
      </div>
    </div>
