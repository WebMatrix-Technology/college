...
export default function HodStudentsPage() {
  ...
  useEffect(() => {
    ...
    if (departmentMockData[dept]) {
      setStudentsData(departmentMockData[dept].students);
      ...
      <div className="mb-10 flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        <div>
          <h1 className="text-3xl md:text-4xl font-black uppercase italic tracking-tighter text-slate-900 flex flex-wrap items-center gap-3">
             ...
             <span className="text-indigo-600">{department}</span> Students
          </h1>
          <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mt-2">Departmental Roster & Academic Tracking</p>
        </div>
            ...
            <div className="flex flex-col sm:flex-row gap-2">
              <div className="relative flex-1 md:flex-none">
                ...
                <input 
                  ...
                  className="w-full md:w-64 pl-9 pr-4 py-2.5 bg-white border-2 border-slate-100 rounded-3xl shadow-sm text-xs font-bold text-slate-600 outline-none focus:border-indigo-500 transition-colors"
                />
              </div>
              <button 
                onClick={() => setIsFilterOpen(!isFilterOpen)}
                className={`px-4 py-2.5 border-2 rounded-3xl shadow-sm transition-colors flex items-center gap-2 text-xs font-bold ${
                  isFilterOpen ? "bg-indigo-50 border-indigo-200 text-indigo-700" : "bg-white border-slate-100 text-slate-500 hover:border-slate-300"
                }`}
              >
              ...
              >
                <div className="flex justify-between items-center mb-4">
                   ...
                   <button 
                     ...
                     className="text-[9px] font-black uppercase tracking-widest text-indigo-600 hover:text-indigo-800 underline"
                   >
                     Clear All
                   </button>
                </div>
                            ...
                            >
                              <td className="p-4">
                                <div className="flex items-center gap-3">
                                  <div className="w-8 h-8 rounded-full bg-indigo-100 text-indigo-500 flex items-center justify-center shrink-0 font-black text-[10px]">
                                    {stu.name.charAt(0)}
                                  </div>
                                  <div>
                                    <p className="text-xs font-black text-slate-900 group-hover:text-indigo-700 transition-colors">{stu.name}</p>
                              ...
                              <td className="p-4 text-right space-x-2">
                                 <button 
                                   ...
                                   className="text-[9px] font-black uppercase tracking-widest text-slate-400 hover:text-indigo-600 transition-colors border border-slate-200 hover:border-indigo-200 px-3 py-1.5 rounded-md bg-white shadow-sm inline-flex items-center gap-1"
                                 >
                                   Profile
                                 </button>
                                 <button 
