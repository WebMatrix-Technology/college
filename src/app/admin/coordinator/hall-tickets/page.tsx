...
export default function CoordinatorHallTicketsPage() {
    ...
    <div className="p-4 md:p-6 lg:p-12 font-sans min-h-screen">
      <div className="mb-10 flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        <div>
          <h1 className="text-3xl md:text-4xl font-black uppercase italic tracking-tighter text-slate-900 flex flex-wrap items-center gap-3">
             <FileText size={36} className="text-fuchsia-600" />
             <span className="text-fuchsia-600">Hall</span> Tickets
          </h1>
        ...
        <div className="flex flex-col sm:flex-row gap-2 w-full md:w-auto mt-4 md:mt-0">
           <motion.button 
             ...
             className="w-full sm:w-auto justify-center bg-fuchsia-600 text-white px-6 py-4 rounded-xl font-black uppercase text-[10px] tracking-widest flex items-center gap-2 hover:bg-fuchsia-700 transition-colors shadow-lg shadow-fuchsia-600/30 disabled:opacity-50 disabled:cursor-not-allowed"
           >
      ...
      <div className="bg-white p-8 rounded-[2rem] border border-slate-100 shadow-xl mb-10">
        <div className="flex flex-col md:flex-row gap-4 mb-8">
          ...
          <div className="relative w-full md:w-64 shrink-0">
            ...
            <select 
              ...
              className="w-full pl-11 pr-4 py-3 bg-slate-50 border-2 border-slate-100 rounded-xl text-sm font-bold text-slate-700 outline-none focus:border-fuchsia-500 transition-colors appearance-none"
            >
        ...
        <div className="flex items-center gap-4 mb-6">
          ...
          <h2 className="text-xl font-black italic uppercase tracking-tighter text-fuchsia-900 flex items-center gap-2">
            <Layers size={20} className="text-fuchsia-500" /> {selectedCourse === "All" ? "All Courses" : selectedCourse} • {selectedBatch === "All" ? "All Years" : selectedBatch}
          </h2>
          <div className="h-px bg-slate-300 flex-1" />
        </div>
              ...
              <AnimatePresence>
                {filteredStudents.map((student, idx) => (
                  <motion.tr 
                    ...
                    key={student.id}
                    className={`transition-colors group ${selectedStudents.includes(student.id) ? 'bg-fuchsia-50/50' : 'hover:bg-slate-50/50'}`}
                  >
                    ...
                    <td className="p-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-xs font-black text-slate-900 group-hover:text-fuchsia-600 transition-colors">{student.name}</p>
                    ...
                    <td className="p-4 text-right space-x-2">
                       {student.ticketStatus === "Pending" && student.feeStatus === "Cleared" && (
                         <button 
                           ...
                           className="text-[9px] font-black uppercase tracking-widest text-fuchsia-600 border border-fuchsia-200 bg-fuchsia-50 hover:bg-fuchsia-600 hover:text-white px-3 py-1.5 rounded-md transition-all shadow-sm inline-flex items-center gap-1"
                         >
                           Generate
                         </button>
                       )}
