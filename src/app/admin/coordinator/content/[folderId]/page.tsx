...
export default function CoordinatorFolderViewPage() {
  ...
  return (
    <div className="p-4 md:p-6 lg:p-12 font-sans min-h-screen">
      ...
      <button 
        onClick={() => router.push("/admin/coordinator/content")}
        className="mb-8 flex items-center gap-2 text-xs font-black uppercase tracking-widest text-slate-500 hover:text-fuchsia-600 transition-colors"
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {mockFiles.map((file, idx) => (
            <motion.div 
              ...
              key={file.id} 
              className="p-6 rounded-2xl border-2 border-slate-100 hover:border-fuchsia-200 hover:bg-fuchsia-50/30 transition-all group flex flex-col gap-4"
            >
              <div className="flex justify-between items-start">
                <div className="w-12 h-12 rounded-xl bg-slate-50 text-slate-500 flex items-center justify-center group-hover:bg-fuchsia-100 group-hover:text-fuchsia-600 transition-colors">
                  <FileText size={24} />
                </div>
                <button 
                  onClick={() => toast(`Downloading ${file.name}`)}
                  className="w-10 h-10 rounded-xl bg-white border border-slate-200 text-slate-400 hover:text-fuchsia-600 hover:border-fuchsia-200 flex items-center justify-center transition-all shadow-sm"
                >
                  <FileDown size={16} />
                </button>
              </div>
              <div>
                <p className="text-sm font-black text-slate-800 break-words group-hover:text-fuchsia-700 transition-colors">
                  {file.name}
                </p>
