...
import { 
  ...
  MapPin, Loader2, BookOpen, CalendarCheck
} from "lucide-react";
...
export default function StudentTimetable() {
  ...
  <VanguardHeader 
    title="Daily" 
    subtitle="Schedule." 
    tag="Node: Timetable // Session 2026" 
    TagIcon={CalendarCheck} 
    primaryColor="orange"
    showBack={true}
    backHref="/students-corner"
  >
    <div className="mt-12 bg-slate-950/40 backdrop-blur-md border border-white/10 p-6 rounded-2xl shadow-2xl w-full md:w-72 flex items-center gap-4 text-white md:absolute md:right-0 md:bottom-0 md:mb-12 md:mr-6">
    <div className="p-3 bg-orange-500 rounded-2xl text-white">
      <CalendarIcon size={24} />
    </div>
    <div>
      <p className="text-[10px] font-black uppercase tracking-widest text-slate-300">Current Day</p>
  ...
  <section className="py-12 max-w-7xl mx-auto px-6 relative z-20 -mt-8">
        ...
        <div className="space-y-4">
          {TIMETABLE_DATA.map((slot, i) => (
            <motion.div
              ...
              key={i}
              className={`border p-6 md:p-8 rounded-[2.5rem] flex flex-col md:flex-row items-start md:items-center gap-6 transition-all group ${
                slot.type === 'Break' 
                  ? 'bg-slate-50 border-slate-100 opacity-60' 
                  : 'bg-white border-slate-200 shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-2xl hover:border-orange-600/30'
              }`}
            >
        ...
        <footer className="mt-20 py-10 border-t border-slate-100 text-center opacity-30">
          <p className="text-[10px] font-black text-slate-400 uppercase tracking-[1em]">Vanguard Time Node // 2026</p>
        </footer>
      </section>
    </main>
  );
