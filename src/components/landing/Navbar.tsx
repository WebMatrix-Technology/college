    ...
    <>
      <nav className="fixed w-full z-[1000] bg-[#101827] border-b border-white/10 py-4 px-6">
        <div className="max-w-[1400px] mx-auto flex justify-between items-center">
          ...
          <div className="hidden lg:flex items-center gap-2 xl:gap-4 2xl:gap-6">
            {navLinks.map((link) => (
              <Link 
                key={link.name} 
                href={link.href}
                className="text-[9px] xl:text-[10px] 2xl:text-[11px] font-extrabold text-white/80 hover:text-orange-500 transition-colors tracking-wider 2xl:tracking-widest whitespace-nowrap"
              >
                {link.name}
              </Link>
            ))}
            
            <Link href="/login" className="bg-orange-600 text-white px-4 py-2 xl:px-6 xl:py-3 text-[9px] xl:text-[10px] 2xl:text-[11px] font-black rounded-sm hover:bg-white hover:text-orange-600 transition-all uppercase tracking-widest whitespace-nowrap">
              Login
            </Link>
          </div>
          ...
          <button 
            className="lg:hidden text-white p-2 hover:bg-white/10 transition-all" 
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed inset-0 bg-[#101827] z-[2000] lg:hidden flex flex-col"
          >
