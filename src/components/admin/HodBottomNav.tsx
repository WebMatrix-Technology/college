...
const primaryLinks = [
  ...
  { name: "Faculty", icon: Users, href: "/admin/hod/faculty" },
  { name: "Students", icon: GraduationCap, href: "/admin/hod/students" },
  { name: "Timetable", icon: Clock, href: "/admin/hod/timetable" },
];
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-slate-900/90 backdrop-blur-xl border-t border-slate-800 pb-safe shadow-[0_-10px_40px_rgba(0,0,0,0.5)]">
        <div className="flex justify-between items-center px-2 py-2">
          {primaryLinks.map((item) => {
            const isActive = pathname === item.href || (pathname.startsWith(item.href) && item.href !== "/admin/hod/dashboard");
            return (
              <Link 
                key={item.name} 
                href={item.href}
              ...
              >
                {isActive && (
                  <motion.div layoutId="nav-pill-hod" className="absolute inset-0 bg-indigo-500/20 rounded-2xl z-0" />
                )}
                <div className="relative z-10 flex flex-col items-center gap-1">
                  <item.icon size={20} className={isActive ? "text-indigo-400" : "text-slate-400 group-hover:text-slate-200"} />
          ...
          >
            ...
            <div className="relative z-10 flex flex-col items-center gap-1">
              <Menu size={20} className={isMoreOpen ? "text-indigo-400" : "text-slate-400"} />
              <span className={`text-[9px] font-black uppercase tracking-wider ${isMoreOpen ? "text-indigo-400" : "text-slate-500"}`}>
                More
              </span>
            </div>
          </button>
            ...
            >
               ...
               <div className="p-2 max-h-[50vh] overflow-y-auto">
                 {secondaryLinks.map((item) => {
                   ...
                   return (
                     <Link key={item.name} href={item.href} onClick={() => setIsMoreOpen(false)}>
                       <div className={`flex items-center gap-4 p-4 rounded-2xl transition-colors ${isActive ? 'bg-indigo-500/10 text-indigo-400' : 'text-slate-300 hover:bg-slate-800'}`}>
                 ...
                 <Link href="/faculty" onClick={() => setIsMoreOpen(false)}>
                   <div className="flex items-center gap-4 p-4 mt-2 rounded-2xl bg-indigo-600/10 text-indigo-400 border border-indigo-500/20 hover:bg-indigo-600/20 transition-colors">
