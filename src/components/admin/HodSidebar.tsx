...
export default function HodSidebar() {
  ...
  useEffect(() => {
    const dept = localStorage.getItem("hodDepartment");
    if (dept) {
      setDepartment(dept);
    }
  }, []);
  ...
  return (
    <div className="w-72 bg-slate-900 text-white min-h-screen flex flex-col font-sans border-r border-slate-800 shadow-2xl relative z-10 overflow-hidden shrink-0">
      ...
      <div className="p-8 shrink-0 relative z-10">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[9px] font-black uppercase tracking-widest text-indigo-400 mt-1">HOD Console</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-6 py-4 custom-scrollbar relative z-10 space-y-1">
        <p className="text-[10px] font-black uppercase tracking-widest text-slate-500 mb-4 px-4">Department Core</p>
        ...
        {navItems.map((item) => {
          ...
          return (
            <Link key={item.name} href={item.href}>
              ...
              >
                {isActive && (
                  <motion.div layoutId="active-pill-hod" className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-6 bg-indigo-500 rounded-r-full" />
                )}
      ...
      <div className="p-6 shrink-0 relative z-10 border-t border-slate-800/50">
         <div className="flex items-center justify-between">
           <div className="flex items-center gap-3">
             <div className="w-10 h-10 bg-slate-800 rounded-full flex items-center justify-center text-slate-400">
               <span className="text-sm font-black">HD</span>
             </div>
             <div>
               <p className="text-xs font-black text-white">{department} HOD</p>
               <p className="text-[9px] font-bold text-slate-400">Department Head</p>
             </div>
           </div>
           <button 
         ...
         <Link href="/faculty" className="mt-4 flex items-center justify-center gap-2 w-full py-2 bg-indigo-600/10 hover:bg-indigo-600/20 text-indigo-400 rounded-xl transition-colors text-[10px] font-black uppercase tracking-widest border border-indigo-500/20">
