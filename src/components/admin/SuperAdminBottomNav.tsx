...
import { 
  LayoutDashboard, Building2, Users, LineChart, 
  Menu, X, CreditCard, Settings, ShieldAlert, 
  Database, LifeBuoy, LogOut
} from "lucide-react";
        ...
        <div className="flex justify-between items-center px-2 py-2">
          {primaryLinks.map((item) => {
            ...
            return (
              ...
              >
                {isActive && (
                  <motion.div layoutId="nav-pill-mobile" className="absolute inset-0 bg-purple-500/20 rounded-2xl z-0" />
                )}
                <div className="relative z-10 flex flex-col items-center gap-1">
                  <item.icon size={20} className={isActive ? "text-purple-400" : "text-slate-400 group-hover:text-slate-200"} />
                  <span className={`text-[9px] font-black uppercase tracking-wider ${isActive ? "text-purple-400" : "text-slate-500"}`}>
                    {item.name}
                  </span>
                </div>
              </Link>
          ...
          >
            ...
            <div className="relative z-10 flex flex-col items-center gap-1">
              <Menu size={20} className={isMoreOpen ? "text-purple-400" : "text-slate-400"} />
              <span className={`text-[9px] font-black uppercase tracking-wider ${isMoreOpen ? "text-purple-400" : "text-slate-500"}`}>
                More
              </span>
            </div>
          </button>
                 ...
                 {secondaryLinks.map((item) => {
                   ...
                   return (
                     <Link key={item.name} href={item.href} onClick={() => setIsMoreOpen(false)}>
                       <div className={`flex items-center gap-4 p-4 rounded-2xl transition-colors ${isActive ? 'bg-purple-500/10 text-purple-400' : 'text-slate-300 hover:bg-slate-800'}`}>
                         <item.icon size={18} className={isActive ? 'text-purple-400' : 'text-slate-400'} />
