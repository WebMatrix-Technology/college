        ...
        <div className="flex justify-between items-center px-2 py-2">
          {primaryLinks.map((item) => {
            ...
            return (
              ...
              >
                {isActive && (
                  <motion.div layoutId="nav-pill-hr" className="absolute inset-0 bg-rose-500/20 rounded-2xl z-0" />
                )}
                 ...
                 {secondaryLinks.map((item) => {
                   ...
                   return (
                     <Link key={item.name} href={item.href} onClick={() => setIsMoreOpen(false)}>
                       <div className={`flex items-center gap-4 p-4 rounded-2xl transition-colors ${isActive ? 'bg-rose-500/10 text-rose-400' : 'text-slate-300 hover:bg-slate-800'}`}>
                         <item.icon size={18} className={isActive ? 'text-rose-400' : 'text-slate-400'} />
