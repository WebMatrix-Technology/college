...
import { 
  ...
  Menu, X, CalendarClock, LogOut
, Sparkles } from "lucide-react";

const primaryLinks = [
  { name: "Dashboard", icon: LayoutDashboard, href: "/admin/hr/dashboard" },
  { name: "Staff", icon: Users, href: "/admin/hr/staff" },
...
export default function HRBottomNav() {
          ...
          {primaryLinks.map((item) => {
            ...
            return (
              ...
              >
                {isActive && (
                  <motion.div 
                    layoutId="nav-pill-hr" 
                    className="absolute inset-0 bg-gradient-to-tr from-rose-600 to-rose-400 rounded-[1.5rem] shadow-[0_0_20px_rgba(0,0,0,0.4)] z-0" 
            ...
            >
               <div className="p-6 border-b border-white/5 flex justify-between items-center relative overflow-hidden">
                 ...
                 <h3 className="text-[10px] font-black uppercase tracking-[0.3em] text-white/50 flex items-center gap-2">
                   <Sparkles size={14} className="text-rose-500" /> Admin Options
                 </h3>
                 ...
                 {secondaryLinks.map((item) => {
                   ...
                   return (
                     <Link key={item.name} href={item.href} onClick={() => setIsMoreOpen(false)}>
                       <div className={`flex flex-col items-start gap-3 p-4 rounded-[1.5rem] border transition-all duration-300 ${isActive ? 'bg-rose-500/10 border-rose-500/20 text-rose-400 shadow-[0_0_15px_rgba(0,0,0,0.2)]' : 'bg-white/5 border-transparent text-white/70 hover:bg-white/10 hover:border-white/10'}`}>
                         <item.icon size={20} strokeWidth={isActive ? 2.5 : 1.5} className={isActive ? 'text-rose-400' : 'text-white/40'} />
